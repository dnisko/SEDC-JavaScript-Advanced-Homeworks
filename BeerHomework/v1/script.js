...
let pageSize = 5;
let beers = null;
let container = document.getElementsByClassName("container text-center")[0];
...
let dataForCards = (data) => {
  ...
  let row = null;
  if (document.getElementsByClassName("row row-cols-4").length > 0) {
    // console.log("ima");
    row = document.getElementsByClassName("row row-cols-4")[0];
  } else {
    // console.log("nema");
...
beersNav.addEventListener("click", () => {
  pagesSort.hidden = false;
  // console.log(url,`?page=${1}`,`&per_page=${pageSize}`);
